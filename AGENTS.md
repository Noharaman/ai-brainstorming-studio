...
## Cross-CLI shared memory

- Codex、Claude Code、Antigravity CLI の共通引継ぎには `.ai-shared/memory.md` を使う。
- 各タスクの開始時に、この `AGENTS.md` の次に `.ai-shared/memory.md` を読み、現在の目標、確定した判断、進捗、次の作業、阻害要因を把握してから回答・作業する。
- `AGENTS.md` は長期的な規則、`.ai-shared/memory.md` は変化する引継ぎ情報である。メモリの記述を鵜呑みにせず、Git状態、実ファイル、一次資料で現在値を確認する。
- ユーザーが今後のAIにも必要な方針、決定、訂正、優先順位を伝えた場合は、コード変更がなくても同じターンでメモリへ反映する。
- 意味のある作業後は、完了内容、検証結果、未完了、次の一手を簡潔に更新し、`Recent handoffs` に日付と使用AIを追記する。
- 書き込み直前にメモリを再読込し、他AIの新しい記述を残す。同時更新や競合を検出した場合は上書きせず、ユーザーへ報告する。
- 秘密情報、認証情報、個人情報、会話全文、未確認の推測は記録しない。記録できない権限・モードの場合は、保存済みだと主張せず明示する。
