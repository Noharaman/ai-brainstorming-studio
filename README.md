...
アプリ自身の会話・セッション記録は `.ai-brainstorm/`、CLI間の共有メモリは `.ai-shared/` へ、実行時に保存します。共有メモリは `.ai-shared/` 内側の `.gitignore` でローカル専用とし、プロジェクトを選択しただけではどちらも書き込みません。

`.ai-shared/memory.md` は、Codex・Claude Code・Antigravity CLI間で確定した指示、判断、進捗、次の作業を引き継ぐためのローカル専用メモリです。Studioは実行開始時にこのファイルを初期化し、`AGENTS.md` とともに共通コンテキストへそのまま含めます。外部CLI 3枠は現在停止中のため、Studioから各CLIへ渡るのは再開後です。
